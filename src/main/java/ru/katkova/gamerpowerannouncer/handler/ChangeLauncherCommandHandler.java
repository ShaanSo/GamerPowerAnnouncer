...
@Service
public class ChangeLauncherCommandHandler implements UserActionHandler {
    ...
    public UserAction getAction() {
        return Command.CHANGE_LAUNCHER;
    }
    ...
    public List<PartialBotApiMethod> handle(User user, Update update) {
        ...
        for (PollQuestions question: PollQuestions.values()) {
            if (question.getHandler().equals("LAUNCHER_HANDLER")) {
        ...
        for (PollOptions option: PollOptions.values()) {
            if (option.getHandler().equals("LAUNCHER_HANDLER")) {
